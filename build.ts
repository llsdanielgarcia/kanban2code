...
async function generateBundledContent() {
  ...
  const providersDir = path.join(sourceRoot, '.kanban2code', '_providers');
  ...
  const providerEntries = await safeReadFilesRecursively(providersDir);
  providerEntries.sort((a, b) => a.relativePath.localeCompare(b.relativePath));
  const bundledProviders: Record<string, string> = {};
  for (const entry of providerEntries) {
    bundledProviders[entry.relativePath] = entry.content;
  }
  ...
  const providersOutput = `// This file is auto-generated by build.ts. Do not edit directly.\n\nexport const BUNDLED_PROVIDERS: Record<string, string> = ${JSON.stringify(bundledProviders, null, 2)} as const;\n`;
  ...
  await fs.writeFile(path.join(assetsDir, 'providers.ts'), providersOutput);
}
