...
/**
 * CLI adapter for Codex (`codex` command).
 *
 * Codex stdin mode requires piping the prompt and passing `-` sentinel.
 * Output is parsed from JSONL events (final text taken from the last text-bearing event).
 */
export class CodexAdapter implements CliAdapter {
  buildCommand(
    ...
    // '-' tells Codex to read prompt text from stdin.
    args.push('-');
